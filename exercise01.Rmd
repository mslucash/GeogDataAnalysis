...
**Geography 495:  Geographic Data Analysis**  
**Spring 2022**

**Exercise 1:  Getting and using R and RStudio**  
**Finish by Friday, April 1st, midnight**
...
2. On the next page, click on "Download R 4.1.3 for Windows" link, and save that file to your hard disk when prompted.  Saving to the desktop is fine.  
...
On the "R for Mac OS X" page ([https://cran.r-project.org/bin/macosx/](https://cran.r-project.org/bin/macosx/)), there are multiple packages that could be downloaded, but there are two choices for "newer" versions of MacOS (There are also packages for older versions):

- `R-4.1.3.pkg` if you're running MacOS High Sierra (10.13) or newer,
- `R-4.1.3.arm64.pkg` if you're running mac with new M1 ARM processor

2. To download the package click on the (e.g.) "`R-4.1.3.pkg`" link.
...
1. Clicking on the Windows Desktop installation package (e.g. `RStudio 2022.02.01 +461` ) will bring up a standard Windows download dialog box.  Save the file to an appropriate place.
2. Open the downloaded file.  
3. Accept the proposed defaults, and quit the installer when finished.  You should see an RStudio icon on the desktop

*Mac*

1. Download the macSO 10.15 disk-image installation (e.g. `RStudio 2022.02.1-461`).  Save the file.
2. Open the downloaded file (`RStudio-2022.02.1-461.dmg`) by clicking on it.  This will open a dialog box that asks if you want to open the file with the default DiskImageMounter application.  
...
After a brief pause, RStudio will open.
	...
	
*Arranging and Running RStudio*

RStudio is flexible enough in its layout (Tools > Global Options > Pane Layout), that individual work habits can be accommodated.  A typical layout might be:

- upper left:  R script (e,g, filename.R) editing window
- lower left:  R Console
- upper right:  a pane with Workspace, History, Files, Packages and Help tabs
- lower right:  Plots pane
...
If you're not in the working directory, you can use the RStudio `Session > Set Working Directory > Choose Working Directory...` dialog.  You can also "strong-arm" the change of the working directory using the `setwd()` function:  
    ...
    setwd("C:/Users/mslucash/Documents/geog495/data/") # Windows
    setwd("C:\\Users\\mslucash\\Documents\\geog495\\data\\") # classical Windows
...
1. right-click on a link to a data set on a web page, like this one:  [[sumcr.csv]](https://github.com/mslucash/GeogDataAnalysis/blob/main/data/csv/sumcr.csv)
2. then save the file. In Windows, click on Raw (button on right), then right click on data and hit "Save as". Navigate to where you want to store the file and use the csv default file type.
...
```{r echo=TRUE, eval=FALSE}
sumcr <- read.csv("/Users/mslucash/Documents/geog495/data/sumcr.csv")  
```
